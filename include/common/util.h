...
template <typename... Args> std::string format(const char* format_str, Args... args) {
...
public:
    ...
    T Pop() {
        ...
        while (m_queue.empty()) {
            // 日志队列为空，线程进入wait状态
            m_condvariable.wait(
                lock); // 这里用unique_lock是因为lock_guard不支持解锁，而unique_lock支持
        }
...
// 技术，即在对象被销毁时会自动解锁。另外， std::unique_lock
// 还支持超时等待和可中断等待的操作。
...
public:
    ...
    int RequestId; // 客户端号码请求的Request的序列号，为了保证线性一致性
                   //  IfDuplicate bool // Duplicate command can't be applied twice , but
                   //  only for PUT and APPEND
...
public:
    friend std::ostream& operator<<(std::ostream& os, const Op& obj) {
        os << "[MyClass:Operation{" + obj.Operation + "},Key{" + obj.Key + "},Value{"
                  + obj.Value + "},ClientId{" + obj.ClientId + "},RequestId{"
