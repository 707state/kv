...
Scheduler::Scheduler(size_t threads, bool use_caller, const std::string& name)
    ...
    if (use_caller) {
        ...
        rootFiber_ = std::make_shared<Fiber>([this] { run(); }, 0, false);
...
void Scheduler::start() {
    ...
    for (size_t i = 0; i < threadCnt_; i++) {
        threadPool_[i] =
            std::make_shared<Thread>([this] { run(); }, name_ + "_" + std::to_string(i));
    ...
    while (true) {
        ...
        if (task.fiber_) {
            ...
            if (cbFiber) {
                ...
                cbFiber = std::make_shared<Fiber>(task.cb_);
            }
            task.reset();
            cbFiber->resume();
            --activeThreadCnt_;
