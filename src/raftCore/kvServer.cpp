...
void KvServer::ExecutePutOpOnKVDB(Op op) {
    ...
    //    DPrintf("[KVServerExePUT----]ClientId :%d ,RequestID :%d ,Key : %v, value : %v",
    //    op.ClientId, op.RequestId, op.Key, op.Value)
    DprintfKVDB();
}
...
void KvServer::Get(const raftKVRpcProctoc::GetArgs* args,
    ...
    m_raftNode->Start(
        ...
        &isLeader); // raftIndex：raft预计的logIndex
                    // ，虽然是预计，但是正确情况下是准确的，op的具体内容对raft来说
                    // 是隔离的
    ...
    if (!chForRaftIndex->timeOutPop(CONSENSUS_TIMEOUT, &raftCommitOp)) {
        ...
        //         DPrintf("[WaitChanGetRaftApplyMessage<--]Server %d , get Command <--
        //         Index:%d , ClientId %d, RequestId %d, Opreation %v, Key :%v, Value
        //         :%v", kv.me, raftIndex, op.ClientId, op.RequestId, op.Operation,
        //         op.Key, op.Value)
        // todo
        // 这里还要再次检验的原因：感觉不用检验，因为leader只要正确的提交了，那么这些肯定是符合的
        if (raftCommitOp.ClientId == op.ClientId
            ...
            reply->set_err(ErrWrongLeader);
            //            DPrintf("[GET ] 不满足：raftCommitOp.ClientId{%v} ==
            //            op.ClientId{%v} && raftCommitOp.RequestId{%v}
            //            == op.RequestId{%v}", raftCommitOp.ClientId, op.ClientId,
            //            raftCommitOp.RequestId, op.RequestId)
        }
    }
    m_mtx.lock(); // todo
                  // 這個可以先弄一個defer，因爲刪除優先級並不高，先把rpc發回去更加重要
...
void KvServer::GetCommandFromRaft(ApplyMsg message) {
    ...
    DPrintf("[KvServer::GetCommandFromRaft-kvserver{%d}] , Got Command --> Index:{%d} , "
            "ClientId {%s}, RequestId {%d}, "
...
void KvServer::PutAppend(const raftKVRpcProctoc::PutAppendArgs* args,
    ...
    if (!isleader) {
        DPrintf("[func -KvServer::PutAppend -kvserver{%d}]From Client %s (Request %d) To "
                "Server %d, key %s, raftIndex %d , but "
                "not leader",
                m_me,
                &args->clientid(),
                args->requestid(),
    ...
    if (!chForRaftIndex->timeOutPop(CONSENSUS_TIMEOUT, &raftCommitOp)) {
        DPrintf("[func -KvServer::PutAppend -kvserver{%d}]TIMEOUT PUTAPPEND !!!! Server "
                "%d , get Command <-- Index:%d , "
...
void KvServer::ReadRaftApplyCommandLoop() {
    while (true) {
        ...
        DPrintf("---------------tmp-------------[func-KvServer::ReadRaftApplyCommandLoop("
                ")-kvserver{%d}] 收到了下raft的消息",
                m_me);
...
//  关于快照raft层与persist的交互：保存kvserver传来的snapshot；生成leaderInstallSnapshot
//  RPC的时候也需要读取snapshot；
//  因此snapshot的具体格式是由kvserver层来定的，raft只负责传递这个东西
//  snapShot里面包含kvserver需要维护的persist_lastRequestId
//  以及kvDB真正保存的数据persist_kvdb
...
KvServer::KvServer(int me, int maxraftstate, std::string nodeInforFileName, short port)
    ...
    std::thread t([this, port]() -> void {
        ...
        provider.NotifyService(
            this->m_raftNode
                .get()); // todo：这里获取了原始指针，后面检查一下有没有泄露的问题
                         // 或者 shareptr释放的问题
    ...
    for (int i = 0; i < INT_MAX - 1; ++i) {
        ...
        ipPortVt.emplace_back(
            nodeIp, atoi(nodePortStr.c_str())); // 沒有atos方法，可以考慮自己实现
    }
    ...
    m_lastSnapShotRaftLogIndex =
        0; // todo:感覺這個函數沒什麼用，不如直接調用raft節點中的snapshot值？？？
    auto snapshot = persister->ReadSnapshot();
    if (!snapshot.empty()) {
        ReadSnapShotToInstall(snapshot);
    }
    std::thread t2(&KvServer::ReadRaftApplyCommandLoop,
                   this); // 马上向其他节点宣告自己就是leader
