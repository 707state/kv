...
void Raft::AppendEntries1(const raftRpcProctoc::AppendEntriesArgs* args,
    ...
    if (matchLog(args->prevlogindex(), args->prevlogterm())) {
        ...
        //                args.PrevLogIndex, reply.UpdateNextIndex, args.PrevLogIndex -
        //                reply.UpdateNextIndex) // 很多都是优化了0
        return;
    }
...
bool Raft::CondInstallSnapshot(int lastIncludedTerm,
    ...
    // rf.mu.Lock()
    // defer rf.mu.Unlock()
    // DPrintf("{Node %v} service calls CondInstallSnapshot with lastIncludedTerm %v and
    // lastIncludedIndex {%v} to check whether snapshot is still valid in term %v", rf.me,
...
void Raft::doElection() {
    ...
    if (m_status == Leader) {
        // fmt.Printf("[       ticker-func-rf(%v)              ] is a Leader,wait the
        // lock\n", rf.me)
    }
    ...
    if (m_status != Leader) {
        ...
        std::shared_ptr<int> votedNum =
            std::make_shared<int>(1); // 使用 make_shared 函数初始化 !! 亮点
        //	重新设置定时器
        m_lastResetElectionTime = now();
        //	发布RequestVote RPC
        for (int i = 0; i < m_peers.size(); i++) {
            ...
            getLastLogIndexAndTerm(&lastLogIndex,
                                   &lastLogTerm); // 获取最后一个log的term和下标
            ...
            std::thread t(&Raft::sendRequestVote,
                          this,
                          i,
                          requestVoteArgs,
                          requestVoteReply,
    ...
    if (m_status == Leader) {
        ...
        for (int i = 0; i < m_peers.size(); i++) {
            ...
            if (m_nextIndex[i] <= m_lastSnapshotIncludeIndex) {
                //                        DPrintf("[func-ticker()-rf{%v}]rf.nextIndex[%v]
                //                        {%v} <= rf.lastSnapshotIncludeIndex{%v},so
                //                        leaderSendSnapShot", rf.me, i, rf.nextIndex[i],
                //                        rf.lastSnapshotIncludeIndex)
                std::thread t(&Raft::leaderSendSnapShot,
                              this,
                              i); // 创建新线程并执行b函数，并传递参数
                t.detach();
                continue;
            ...
            if (preLogIndex != m_lastSnapshotIncludeIndex) {
                for (int j = getSlicesIndexFromLogIndex(preLogIndex) + 1;
                    ...
                    *sendEntryPtr =
                        m_logs[j]; //=是可以点进去的，可以点进去看下protobuf如何重写这个的
                }
            } else {
                for (const auto& item: m_logs) {
                    raftRpcProctoc::LogEntry* sendEntryPtr =
                        appendEntriesArgs->add_entries();
                    *sendEntryPtr =
                        item; //=是可以点进去的，可以点进去看下protobuf如何重写这个的
                }
            }
            int lastLogIndex = getLastLogIndex();
            // leader对每个节点发送的日志长短不一，但是都保证从prevIndex发送直到最后
            myAssert(appendEntriesArgs->prevlogindex() + appendEntriesArgs->entries_size()
                         == lastLogIndex,
                     format("appendEntriesArgs.PrevLogIndex{%d}+len(appendEntriesArgs."
                            "Entries){%d} != lastLogIndex{%d}",
            ...
            std::thread t(&Raft::sendAppendEntries,
                          this,
                          i,
                          appendEntriesArgs,
                          appendEntriesReply,
...
void Raft::electionTimeOutTicker() {
    ...
    while (true) {
        ...
        if (std::chrono::duration<double, std::milli>(suitableSleepTime).count() > 1) {
            ...
            usleep(
                std::chrono::duration_cast<std::chrono::microseconds>(suitableSleepTime)
                    .count());
        ...
        if (std::chrono::duration<double, std::milli>(m_lastResetElectionTime - wakeTime)
                .count()
            > 0) {
...
std::vector<ApplyMsg> Raft::getApplyLogs() {
    ...
    while (m_lastApplied < m_commitIndex) {
        m_lastApplied++;
        myAssert(m_logs[getSlicesIndexFromLogIndex(m_lastApplied)].logindex()
                     == m_lastApplied,
                 format("rf.logs[rf.getSlicesIndexFromLogIndex(rf.lastApplied)].LogIndex{"
                        "%d} != rf.lastApplied{%d} ",
...
void Raft::InstallSnapshot(const raftRpcProctoc::InstallSnapshotRequest* args,
    ...
    if (args->term() < m_currentTerm) {
        ...
        //        leader{%v}.term{%v}<rf{%v}.term{%v} ", rf.me, args.LeaderId, args.Term,
        //        rf.me, rf.currentTerm)
    ...
    if (lastLogIndex > args->lastsnapshotincludeindex()) {
        m_logs.erase(m_logs.begin(),
                     m_logs.begin()
                         + getSlicesIndexFromLogIndex(args->lastsnapshotincludeindex())
                         + 1);
    } else {
        m_logs.clear();
    }
    ...
    std::thread t(
        &Raft::pushMsgToKvServer, this, msg); // 创建新线程并执行b函数，并传递参数
    t.detach();
    // 看下这里能不能再优化
...
void Raft::leaderHearBeatTicker() {
    while (true) {
        ...
        if (std::chrono::duration<double, std::milli>(m_lastResetHearBeatTime - wakeTime)
                .count()
            > 0) {
...
void Raft::leaderUpdateCommitIndex() {
    ...
    for (int index = getLastLogIndex(); index >= m_lastSnapshotIncludeIndex + 1;
        ...
        // log.Printf("lastSSP:%d, index: %d, commitIndex: %d, lastIndex:
        // %d",rf.lastSSPointIndex, index, rf.commitIndex, rf.getLastIndex())
...
bool Raft::matchLog(int logIndex, int logTerm) {
    myAssert(logIndex >= m_lastSnapshotIncludeIndex && logIndex <= getLastLogIndex(),
             format("不满足：logIndex{%d}>=rf.lastSnapshotIncludeIndex{%d}&&logIndex{%d}<"
                    "=rf.getLastLogIndex{%d}",
                    logIndex,
                    m_lastSnapshotIncludeIndex,
                    logIndex,
                    getLastLogIndex()));
...
void Raft::RequestVote(const raftRpcProctoc::RequestVoteArgs* args,
    ...
    if (!UpToDate(args->lastlogindex(), args->lastlogterm())) {
        // args.LastLogTerm < lastLogTerm || (args.LastLogTerm == lastLogTerm &&
        // args.LastLogIndex < lastLogIndex) {
        // 日志太旧了
        if (args->lastlogterm() < lastLogTerm) {
            //                    DPrintf("[	    func-RequestVote-rf(%v)		] : refuse
            //                    voted rf[%v] ,because candidate_lastlog_term{%v} <
            //                    lastlog_term{%v}\n", rf.me, args.CandidateId,
            //                    args.LastLogTerm, lastLogTerm)
        } else {
            //            DPrintf("[	    func-RequestVote-rf(%v)		] : refuse voted
            //            rf[%v] ,because candidate_log_index{%v} < log_index{%v}\n",
    ...
    if (m_votedFor != -1 && m_votedFor != args->candidateid()) {
        //        DPrintf("[	    func-RequestVote-rf(%v)		] : refuse voted rf[%v]
        //        ,because has voted\n", rf.me, args.CandidateId)
        reply->set_term(m_currentTerm);
        reply->set_votestate(Voted);
        reply->set_votegranted(false);
...
int Raft::getLogTermFromLogIndex(int logIndex) {
    ...
    myAssert(
        ...
        format(
            "[func-getSlicesIndexFromLogIndex-rf{%d}]  logIndex{%d} > lastLogIndex{%d}",
            m_me,
            logIndex,
            lastLogIndex));
...
bool Raft::sendRequestVote(int server,
    ...
    DPrintf(
        "[func-sendRequestVote rf{%d}] 向server{%d} 發送 RequestVote 完畢，耗時:{%d} ms",
        m_me,
        m_currentTerm,
        getLastLogIndex(),
        now() - start);
    ...
    if (*votedNum >= m_peers.size() / 2 + 1) {
        ...
        *votedNum = 0;
        if (m_status == Leader) {
            ...
            myAssert(false,
                     format("[func-sendRequestVote-rf{%d}]  term:{%d} "
                            "同一个term当两次领导，error",
                            m_me,
                            m_currentTerm));
        }
...
bool Raft::sendAppendEntries(int server,
    ...
    //  ,直到这个log成功的被store
    DPrintf("[func-Raft::sendAppendEntries-raft{%d}] leader 向节点{%d}发送AE rpc開始 ， "
            "args->entries_size():{%d}",
            m_me,
            server,
            args->entries_size());
    ...
    if (!ok) {
        DPrintf("[func-Raft::sendAppendEntries-raft{%d}] leader 向节点{%d}发送AE rpc失敗",
                m_me,
                server);
        return ok;
    }
    DPrintf("[func-Raft::sendAppendEntries-raft{%d}] leader 向节点{%d}发送AE rpc成功",
            m_me,
            server);
    ...
    if (reply->term() > m_currentTerm) {
        ...
        DPrintf(
            "[func -sendAppendEntries  rf{%d}]  节点：{%d}的term{%d}<rf{%d}的term{%d}\n",
            m_me,
            server,
            reply->term(),
            m_me,
    ...
    if (!reply->success()) {
        ...
        *appendNums = *appendNums + 1;
        ...
        myAssert(m_nextIndex[server] <= lastLogIndex + 1,
                 format("error msg:rf.nextIndex[%d] > lastLogIndex+1, len(rf.logs) = %d  "
                        " lastLogIndex{%d} = %d",
                        server,
                        m_logs.size(),
                        server,
        ...
        if (*appendNums >= 1 + m_peers.size() / 2) {
            ...
            *appendNums = 0;
            ...
            if (args->entries_size() > 0
                ...
                m_commitIndex =
                    std::max(m_commitIndex, args->prevlogindex() + args->entries_size());
            }
...
void Raft::init(std::vector<std::shared_ptr<RaftRpcUtil>> peers,
    ...
    m_ioManager = std::make_unique<IOManager>(FIBER_THREAD_NUM, FIBER_USE_CALLER_THREAD);
...
void Raft::Snapshot(int index, std::string snapshot) {
    ...
    if (m_lastSnapshotIncludeIndex >= index || index > m_commitIndex) {
        DPrintf("[func-Snapshot-rf{%d}] rejects replacing log with snapshotIndex %d as "
                "current snapshotIndex %d is larger or "
                "smaller ",
                m_me,
                index,
                m_lastSnapshotIncludeIndex);
