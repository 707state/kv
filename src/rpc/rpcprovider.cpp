...
void RpcProvider::Run(int nodeIndex, short port) {
    // Setup Boost.Asio TCP acceptor
    boost::asio::ip::tcp::endpoint endpoint(boost::asio::ip::tcp::v4(), port);
    m_acceptor =
        std::make_shared<boost::asio::ip::tcp::acceptor>(*m_io_context, endpoint);
...
void RpcProvider::OnConnection(
    ...
    // Read message from client asynchronously
    auto buffer =
        std::make_shared<std::vector<char>>(1024); // Adjust buffer size as needed
    socket->async_read_some(boost::asio::buffer(*buffer),
                            [this, socket, buffer](const boost::system::error_code& error,
                                                   std::size_t bytes_transferred) {
                                if (!error) {
                                    OnMessage(socket, error, bytes_transferred);
                                }
                            });
}

void RpcProvider::OnMessage(const std::shared_ptr<boost::asio::ip::tcp::socket>& socket,
                            const boost::system::error_code& error,
                            std::size_t bytes_transferred) {
    // Process received message here
    // After processing, you can prepare an RPC response
    if (!error) {
        // Example of sending a response (message logic omitted for simplicity)
        google::protobuf::Message* response = nullptr; // Create the appropriate response
        SendRpcResponse(socket, response);
    }
}
...
void RpcProvider::SendRpcResponse(
    ...
    // Serialize the protobuf response and send it
    std::string response_str = response->SerializeAsString();
    auto buffer =
        std::make_shared<std::vector<char>>(response_str.begin(), response_str.end());
    boost::asio::async_write(*socket,
                             boost::asio::buffer(*buffer),
                             [socket, buffer](const boost::system::error_code& error,
                                              std::size_t bytes_transferred) {
